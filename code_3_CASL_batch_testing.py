...
def run_grid_search(train_data, test_data, param_grid):
    results = []
    
    for params in ParameterGrid(param_grid):
        print(f"Running with params: {params}")
        
        num_clu = params['num_clu']
        rhopara = params['rhopara']
        lampara = params['lampara']
        wpara0 = params['wpara0']
        cpara = [9]*num_clu
        
        val_err_mix, y_val_hat, clu_val_hat, betavals, beta0val, val_errS0, val_errS1, val_count_S0, val_count_S1 = AM_Alg(
            train_data, test_data, batch_num, wpara0, wpara1, rhopara, num_clu, ratioClu, lampara, mixpara, cpara)
        ...
        results.append({
            ...
            #'betavals': betavals,
            #'beta0val': beta0val,
            #'clu_val_hat': clu_val_hat,
            #'y_val_hat': y_val_hat
        })
        
        save_results_to_csv(results, f"results_grid_search_2020.csv")

    print("Grid search complete. Results saved to results_grid_search.csv")
...
run_grid_search(retrain_data, test_data, param_grid)
