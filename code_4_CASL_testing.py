...
val_err_mix, test_ypred, val_clu, val_beta, val_beta0, fpr, mae, count_S0, count_S1  = AM_Alg(retrain_data,test_data,batch_num,wpara0,wpara1,
                                                                   best_rhopara,best_num_clu,ratioClu,best_lampara,mixpara,cpara)


# ----------------- Save Results -----------------

def test_result(test_data, test_ypred, val_clu):
    result_df = pd.DataFrame({
        'county_fips': test_data.index,  # The county fips (assumed to be the index of the DataFrame)
        'clustering_result': val_clu,
...
df_test_result = test_result(test_data, test_ypred, val_clu)
df_test_coeff = test_coeff(test_data, val_beta, val_beta0)
df_test_error = test_error(test_data, test_ypred, mixpara)


df_test_result.to_csv('testing_results_'+str(batch_test_year)+'_1107_v1_4clu.csv',index=False)
df_test_coeff.to_csv('testing_coeff_'+str(batch_test_year)+'_1107_v1_4clu.csv')
df_test_error.to_csv('testing_error_'+str(batch_test_year)+'_1107_v1_4clu.csv')
